...
class NotificationReadControllerIntegrationTest : WebIntegrationTestSupport() {
    ...
    fun inbox_filters_by_method_and_status() {
        val sentFcm = persistencePort.save(requested("sent-fcm", NotificationMethod.FCM).markSent(java.time.LocalDateTime.now()))
        ...
        mockMvc.perform(get("/api/notifications").with(user(userDetails)))
            ...
            .andExpect(jsonPath("$.data[0].senderNickname").value("sender"))
    ...
    fun mark_as_read_updates_notification() {
        val sent = persistencePort.save(requested("read-target", NotificationMethod.FCM).markSent(java.time.LocalDateTime.now()))
    ...
    private fun requested(key: String, method: NotificationMethod): Notification =
        Notification.request(
            ...
            targetIdentifier = userDetails.email,
            method = method,
            senderEmail = "sender@example.com",
            rendered = NotificationType.FRIEND_REQUEST_RECEIVED.render(
