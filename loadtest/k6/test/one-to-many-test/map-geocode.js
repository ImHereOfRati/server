...
const users = new SharedArray('imhere-only-one-map-user', () =>
...
const statuses = new Counter('only_one_map_response_count');
const rateLimited = new Counter('only_one_map_rate_limit_count');
const serverErrors = new Counter('only_one_map_server_error_count');
const networkErrors = new Counter('only_one_map_network_error_count');
const duration = new Trend('only_one_map_request_duration', true);
...
export default function () {
  const response = http.get(`${baseUrl}/api/maps/geocode?query=%EC%97%AD%EC%82%BC%EC%97%AD`, {
    headers: {
      Authorization: `Bearer ${actor.accessToken}`,
      'Content-Type': 'application/json',
    },
    tags: { endpoint: 'only-one-map-geocode' },
  });
  ...
  check(response, { 'only-one map request completed': (r) => r.status !== 0 });
}
