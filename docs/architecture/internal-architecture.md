...
| 외부 연동이 많은 영역만 포트/어댑터를 강화 | OIDC, JWT, Modulith event, FCM/SMS 연동부는 adapter/port 분리를 쓴다 | 테스트 격리와 교체 가능성이 필요하다 |
...
| `notifications` | 강한 헥사고날 성향 | Modulith event, FCM/SMS, persistence, DEAD 재발송이 포트 경계를 쓴다 |
...
2. `notifications` 는 HTTP 요청과 모듈 도메인 이벤트가 같은 `Notification` 상태 머신을 공유한다.
