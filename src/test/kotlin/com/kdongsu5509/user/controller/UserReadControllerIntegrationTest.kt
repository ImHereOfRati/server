...
class UserReadControllerIntegrationTest : WebIntegrationTestSupport() {
    ...
    fun readAllUserForbidden() {
        // GET /api/users 와 GET /api/users?keyword=... 의 라우팅은
        // 파라미터 유무에 따라 Spring MVC가 매핑을 결정합니다.
        // param 없이 /api/users 호출 시 readAll이 호출되며, @PreAuthorize("hasRole('ADMIN')")에 의해 403이 발생해야 합니다.
        mockMvc.perform(
            get("/api/users")
                .with(user(myDetails))
